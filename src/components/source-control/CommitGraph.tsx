...
function graphColor(colorIndex: number): string {
  return GRAPH_LANE_COLORS[colorIndex];
}
...
export default function CommitGraph({ row, isHead = false }: CommitGraphProps) {
  ...
  if (inputIndex !== -1) {
    paths.push(<path key="into-node" d={`M ${LANE_WIDTH * (circleIndex + 1)} 0 V ${ROW_HEIGHT / 2}`} fill="none" stroke={graphColor(input[inputIndex].colorIndex)} strokeLinecap="round" strokeWidth={1} />);
  }
  if (row.parentCount > 0) {
    paths.push(<path key="out-node" d={`M ${LANE_WIDTH * (circleIndex + 1)} ${ROW_HEIGHT / 2} V ${ROW_HEIGHT}`} fill="none" stroke={graphColor(circleColor)} strokeLinecap="round" strokeWidth={1} />);
  }

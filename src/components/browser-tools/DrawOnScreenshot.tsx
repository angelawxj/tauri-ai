...
export default function DrawOnScreenshot({ surfaceRef, disabled }: BrowserToolProps) {
  ...
  const [base, setBase] = useState<string | null>(null); const [tool, setTool] = useState<Tool>("pen"); const [color, setColor] = useState(COLORS[0]); const [width, setWidth] = useState(4); const [fontSize, setFontSize] = useState(18); const [styleOpen, setStyleOpen] = useState(false); const [fontOpen, setFontOpen] = useState(false); const [shapes, setShapes] = useState<Shape[]>([]); const [future, setFuture] = useState<Shape[]>([]); const current = useRef<Shape | null>(null);
  ...
  const down = (event: React.PointerEvent<HTMLCanvasElement>) => { if (tool === "text") { const text = window.prompt("输入注释文字"); if (text) { const at = point(event); setShapes((value) => [...value, { tool, color, width: fontSize / 5, from: at, to: at, points: [], text }]); setFuture([]); } return; } const at = point(event); current.current = { tool, color, width, from: at, to: at, points: [at] }; event.currentTarget.setPointerCapture(event.pointerId); };
  ...
  return <>
    ...
    {base && <div className="browser-markup-overlay">
      ...
      <div className="browser-markup-toolbar">{tools.map(([id, icon, label]) => <button key={id} title={label} className={tool === id ? "active" : ""} onClick={() => setTool(id)}>{icon}</button>)}<i />
        <div className="relative"><button title="颜色和粗细" onClick={() => { setStyleOpen((v) => !v); setFontOpen(false); }}><span className="block size-5 rounded-full border border-gray-300" style={{ background: color }} /></button>{styleOpen && <div className="absolute bottom-10 left-0 z-10 w-44 rounded-lg border border-gray-200 bg-white p-2 shadow-lg"><div className="browser-color-row">{COLORS.map((value) => <button key={value} aria-label={value} className={color === value ? "active" : ""} style={{ background: value }} onClick={() => { setColor(value); setStyleOpen(false); }} />)}</div><div className="mt-2 flex gap-1">{[2,4,8].map((value) => <button key={value} className="flex h-7 flex-1 items-center justify-center rounded border border-gray-200" onClick={() => { setWidth(value); setStyleOpen(false); }}><span className="browser-width-dot" style={{ width: value + 2, height: value + 2 }} /></button>)}</div></div>}</div>
        <div className="relative"><button title="文字大小" onClick={() => { setFontOpen((v) => !v); setStyleOpen(false); }}><span className="flex items-center gap-1"><Type size={15} />{fontSize}</span></button>{fontOpen && <div className="absolute bottom-10 left-0 z-10 flex rounded-lg border border-gray-200 bg-white p-2 shadow-lg">{[14,18,24,32,48].map((value) => <button key={value} className={fontSize === value ? "active" : ""} onClick={() => { setFontSize(value); setFontOpen(false); }}>{value}</button>)}</div>}</div><i />

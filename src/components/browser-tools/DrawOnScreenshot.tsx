...
import { ArrowUpRight, Check, Circle, Highlighter, Paintbrush, Pencil, Redo2, Square, Trash2, Type, Undo2, X } from "lucide-react";
...
function drawShape(context: CanvasRenderingContext2D, shape: Shape | null) {
  if (!shape) return;
...
export default function DrawOnScreenshot({ surfaceRef, disabled }: BrowserToolProps) {
  ...
  const start = async () => { const surface = surfaceRef.current; if (!surface) return; const image = await captureSurface(surface); if (image) { setBase(image); setShapes([]); setFuture([]); await waitForPaint(); await setNativeSurfaceVisible(surface, false).catch(() => {}); } };
  ...
  return <>
    ...
    {base && <div className="browser-markup-overlay">
      ...
      <div className="browser-markup-toolbar">{tools.map(([id, icon, label], index) => <span key={id} className="browser-markup-tool-wrap"><button title={label} className={tool === id ? "active" : ""} onClick={() => setTool(id)}>{icon}</button>{index === 0 && <button className="browser-markup-popover" aria-label="画笔选项"><Square size={14} /></button>}</span>)}<i />
        <button className="browser-current-color" title="切换颜色" style={{ background: color }} onClick={() => setColor(COLORS[(COLORS.indexOf(color) + 1) % COLORS.length])} />
        <button className="browser-text-size" title="线条粗细" onClick={() => setWidth(width === 8 ? 2 : width * 2)}><Type size={17} /><b>{Math.max(12, width * 4 + 2)}</b></button><i />
        ...
        <button title="清空" disabled={!shapes.length} onClick={() => { setFuture([...shapes].reverse()); setShapes([]); }}><Trash2 size={18} /></button>
      </div>
      <div className="browser-markup-actions"><span>在页面上绘制，然后复制标注以粘贴到你的智能体。</span><button onClick={() => void close()}><X size={18} />取消</button><button className="browser-tool-primary" onClick={() => void finish()}><Check size={18} />复制标注</button></div>
    </div>}
  </>;
}
